...
<body>
    ...
    <div class="container-fluid tm-container-content tm-mt-60">
        ...
        <h1>Modifier ou supprimer cet évènement </h1>
        <form action="index.php?folder=EventController&amp;file=directionUpdateValidateEvent" method="POST" class="form">
            <div class="row">
                <div class="col-6">
                    <label for="name">Titre</label>
                    <input type="hidden" id="idEvent" name="idEvent" value="<?=$oneEvent->id()?>">
                    <input type="text" class="form-control" placeholder="Titre" id="name" name="name" value="<?=$oneEvent->name()?>" required>
                </div>
                <div class="col-6">
                    <label for="date">Date</label>
                    <input type="date" class="form-control" placeholder="date" id="date" name="date" value="<?=$date?>" required>
                </div>
            </div>
            <div class="row">
                <div class="col-6">
                    <label for="start">Heure de début</label>
                    <input type="time" class="form-control" require placeholder="HH:MM" id="start" name="start" value="<?=$startHour?>" required>
                </div>
                <div class="col-6">
                    <label for="end">Heure de fin</label>
                    <input type="time" class="form-control" require placeholder="HH:MM" id="end" name="end" value="<?=$endHour?>" required>
                </div>
            </div>
            <div class="row">
                <div class="col">
                    <label for="description">Description</label>
                    <textarea class="form-control" placeholder="Description" id="description" name="description" value="<?=$oneEvent->description()?>" required><?=$oneEvent->description()?></textarea>
                </div> 
            </div>
            <br>
            <div class="row">
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Modification d'un évènement</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                <p>Vous êtes sur le point de modifier cet évènement.</p>
                ...
                <div class="modal fade" id="exampleModal2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Suppression d'un évènement</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                <p>Vous êtes sur le point de supprimer cet évènement.</p>
                                <p>Êtes-vous sur de vouloir faire cette action ?</p>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Fermer</button>
                                <input type="submit" value="Supprimer" class="btn btn-danger" name="Supprimer" data-toggle="modal" data-target="#exampleModal2">
                            </div>
                        </div>
                    </div>
                </div>   
